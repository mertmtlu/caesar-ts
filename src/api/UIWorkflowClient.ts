...
export class UIWorkflowClient implements interfaces.IUIWorkflowClient {
     ...
     * @return OK
     */
    uIWorkflow_CompleteUIInteraction(interactionId: string, body: { [key: string]: any; } | undefined): Promise<types.NodeExecutionResponseDtoApiResponse> {
        let url_ = this.baseUrl + "/api/UIWorkflow/interactions/{interactionId}/complete";
        if (interactionId === undefined || interactionId === null)
            throw new Error("The parameter 'interactionId' must be defined.");
        url_ = url_.replace("{interactionId}", encodeURIComponent("" + interactionId));
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processUIWorkflow_CompleteUIInteraction(_response);
        });
    }

    protected processUIWorkflow_CompleteUIInteraction(response: Response): Promise<types.NodeExecutionResponseDtoApiResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
