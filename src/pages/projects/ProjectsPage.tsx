  ...
  const loadProjects = async () => {
    try {
      ...
      if (response.success && response.data) {
        console.log('Raw API response data:', response.data.items);
        const projectItems = response.data.items?.map(project => {
          console.log('Project data:', project.name, 'isPublic:', project.isPublic);
          return {
            id: project.id || '',
        ...
        console.log('Final projectItems array:', projectItems.map(p => ({ name: p.name, isPublic: p.isPublic })));
          ...
          >
            <div className="p-6">
              <div className="flex items-start justify-between">
                ...
                {project.isPublic ? (
                  <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700/50 shadow-sm" title="Public project - discoverable by all authenticated users">
                  ...
                  <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-gray-50 text-gray-700 border border-gray-200 dark:bg-gray-800/70 dark:text-gray-300 dark:border-gray-600/50 shadow-sm" title="Private project - restricted access only">
                    <svg className="w-3 h-3 mr-1.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
