...
const DesignCanvas: React.FC<DesignCanvasProps> = ({
  ...
  const renderElementContent = (element: UIElement) => {
    ...
    switch (element.type) {
      ...
      case 'map_input':
        return (
          <div className="w-full h-full flex flex-col items-center justify-center bg-gray-200 dark:bg-gray-600 overflow-hidden">
            <svg className="w-1/4 h-1/4 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400 font-semibold">{element.label}</p>
            <p className="text-[10px] text-gray-400 dark:text-gray-500">Map Preview</p>
          </div>
        );
      default:
        return (
